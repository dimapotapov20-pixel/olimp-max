...
5. Обычный кандидат проверяет ревьюер. Для строгого адаптера или полностью размеченной строки обычной таблицы действует второй путь: строка публикуется сама, только если точно разрешились программа конкретной площадки и профиль РСОШ.
...
сопоставить программу с записью в БД и подтвердить правило. Исключение —
обычная табличная строка, в которой в отдельных ячейках прямо указаны
программа, олимпиада, профиль, статус диплома и БВИ/100 баллов. Она получает
`confidence = 100` и идёт в тот же автоматический издатель; совпадение
программы и профиля РСОШ всё равно проверяется заново. Свободный текст,
объединённые ячейки PDF и формулировки «все направления» никогда не проходят
автоподтверждение.

## Автоматическое подтверждение без ручного ревью
...
У МГУ есть отдельный разборщик таблицы «программа → олимпиада → профиль →
льгота»: он раскрывает только прямо перечисленные названия олимпиад и не
раскрывает `*` («все олимпиады соответствующего уровня»). Название программы
из первой колонки может быть добавлено в каталог только как точная цитата
официальной строки. У Бауманки приложения 5.1 (БВИ) и 5.3 (100 баллов)
запускаются разными адаптерами. Автопубликация из 5.1 возможна только для
явно указанного кода направления; приложение 5.3 и страница МФТИ пока
останавливаются до добавления первичного сопоставления «конкурсная группа /
поле подготовки → программа». Это предохранитель, а не отсутствие данных.
...
Он не появится у пользователя. Строка из универсальной таблицы дополнительно
должна иметь техническую метку `automatic_explicit_table_row`: это защита от
случайной «уверенности 100» в другом коде. Автоматическая строка получает
